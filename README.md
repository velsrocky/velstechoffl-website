...
node tools/gen-search-index.js    # Prebuilt search index (106 entries)
...
- **Search** (`tools/gen-search-index.js` → `search-index.json` 106 entries, `sw.js` precached network-first) – fuzzy ranked in `script.js` (`editDist1` incl. transposition, tag/category/desc weighting, top-8, `↑`/`↓`/`Enter`/`Esc`, `?q=` param).
